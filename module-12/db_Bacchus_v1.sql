...
DROP TABLE IF EXISTS wines_grapes_join;
...
CREATE TABLE grapes (
    grape_ID     INT             NOT NULL    AUTO_INCREMENT,
    grape_name   VARCHAR(25)     NOT NULL,
    
    
    PRIMARY KEY(grape_ID)
);

CREATE TABLE wines_grapes_join (
...
-- insert into grapes table
INSERT INTO wines_grapes_join(wine_ID,grape_ID)
    VALUES(
        
        (SELECT wine_ID 
        FROM wines 
        WHERE wine_name = 'Merlot'),
...
INSERT INTO wines_grapes_join(wine_ID,grape_ID)
    VALUES(
        ...
        (SELECT wine_ID 
        FROM wines 
        WHERE wine_name = 'Cabernet'),
       
        (SELECT grape_ID 
        FROM grapes 
        WHERE grape_name = 'Cabernet')
    );
